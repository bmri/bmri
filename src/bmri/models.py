...
def generator_type1(Input_W=64,Input_H=64,Input_C=1,Z_Dim=50,k0=32,reg_param=1.0e-7):
...
def generator_type2(Input_W=64,Input_H=64,Input_C=1,k0=32,reg_param=1.0e-7):
  input_X = keras.Input(shape=(Input_W,Input_H,Input_C))
  input_Z = keras.Input(shape=(Input_W,Input_H,1))
  input_XZ = keras.layers.Concatenate()([input_X,input_Z])

  # Downsampling + ResBlock
  X1  = DownSample(input_X=input_XZ,k=k0,downsample=False,reg_param=reg_param)
  X1  = ResBlock(input_X=X1,reg_param=reg_param)
  
  X2  = DownSample(input_X=X1,k=2*k0,reg_param=reg_param)
  X2  = ResBlock(input_X=X2,normalization='bn',reg_param=reg_param)

  X3  = DownSample(input_X=X2,k=4*k0,reg_param=reg_param)
  X3  = ResBlock(input_X=X3,normalization='bn',reg_param=reg_param)

  # Final downsampling + ResBlock (not to be concatenated)
  X4  = DownSample(input_X=X3,k=8*k0,reg_param=reg_param)
  X4  = ResBlock(input_X=X4,normalization='bn',reg_param=reg_param)

  # Coarsest level ResBlock
  X5  = ResBlock(input_X=X4,normalization='bn',reg_param=reg_param)
  
  # Upsampling + ResBlock
  X6  = UpSample(input_X=X5,k=8*k0,reg_param=reg_param)
  X6  = ResBlock(input_X=X6,normalization='bn',reg_param=reg_param)

  # Upsampling + Concat + ResBlock
  X7  = UpSample(input_X=X6,k=4*k0,concat=True,old_X=X3,reg_param=reg_param)
  X7  = ResBlock(input_X=X7,normalization='bn',reg_param=reg_param)

  X8  = UpSample(input_X=X7,k=2*k0,concat=True,old_X=X2,reg_param=reg_param)
  X8  = ResBlock(input_X=X8,normalization='bn',reg_param=reg_param)

  X9  = UpSample(input_X=X8,k=k0,concat=True,old_X=X1,upsample=False,reg_param=reg_param)
  X9  = ResBlock(input_X=X9,normalization='bn',reg_param=reg_param)
...
def critic(Input_W=64,Input_H=64,Input_C=1,k0=32,reg_param=1.0e-7):
  ...
  # Downsampling + ResBlock
  X1  = DownSample(input_X=input_XY,k=k0,downsample=False,reg_param=reg_param)
  X1  = ResBlock(input_X=X1,reg_param=reg_param)
  
  X2  = DownSample(input_X=X1,k=2*k0,reg_param=reg_param)
  X2  = ResBlock(input_X=X2,normalization='ln',reg_param=reg_param)

  X3  = DownSample(input_X=X2,k=4*k0,reg_param=reg_param)
  X3  = ResBlock(input_X=X3,normalization='ln',reg_param=reg_param)

  X4  = DownSample(input_X=X3,k=8*k0,reg_param=reg_param)
  X4  = ResBlock(input_X=X4,normalization='ln',reg_param=reg_param)
...
def critic_be(Input_W=64,Input_H=64,Input_C=1,k0=32,reg_param=1.0e-7, dense_flat_size = 64):
  ...
  X6  = keras.layers.Dense(units=dense_flat_size,
                           activation=activation_func,
                           kernel_regularizer=keras.regularizers.l2(reg_param),
                           use_bias=use_bias)(X5)
  X7  = keras.layers.LayerNormalization()(X6)                         
